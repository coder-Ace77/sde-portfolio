...
A person in city 1 wants to reach city N. To reach there there are two ways either he can go to next city using bus using `B[i]` cost. we can fly to any of then next k cities by giving cost `A[i]+A[j]` where `j<=i-k` 
